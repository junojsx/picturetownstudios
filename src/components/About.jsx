...
const timeline = [
  { year: '2024', role: 'Director / Founder',    company: 'PictureTown Studios — Los Angeles', current: true  },
  { year: '2021', role: 'Senior Director',        company: 'Luminary Films, Los Angeles' },
  { year: '2018', role: 'Director / DOP',         company: 'Atlas Creative Group, New York City' },
  { year: '2015', role: 'Commercial Director',    company: 'Freelance · Branded Content' },
  { year: '2012', role: 'MFA Film Production',    company: 'UCLA School of Theater, Film & TV' },
]
...
export default function About() {
  return (
    <section id="about" className="py-24 md:py-32 bg-brand-black overflow-hidden">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-[1fr_1.3fr] gap-12 lg:gap-20 items-start">
          ...
          <div className="relative max-w-sm mx-auto lg:mx-0 w-full">
            ...
            <div className="relative aspect-[3/4] overflow-hidden bg-brand-card">
              <img
                src="https://picsum.photos/seed/director-portrait-am/600/800"
                alt="Alex Mercer — Director & Producer"
                className="absolute inset-0 w-full h-full object-cover grayscale"
                style={{ mixBlendMode: 'luminosity', opacity: 0.65 }}
              />
              {/* Coloured rim overlay */}
              <div className="absolute inset-0 bg-[radial-gradient(ellipse_at_100%_40%,rgba(58,124,165,0.25)_0%,transparent_60%)]" />
            ...
            <div className="absolute bottom-6 left-6 bg-brand-card/95 backdrop-blur-md border border-white/[0.08] px-5 py-4">
              <div className="font-display text-brand-amber text-5xl leading-none">12+</div>
          ...
          <div className="flex flex-col justify-center">
            ...
            {/* Name / title */}
            <h2 className="font-display leading-[0.88]" style={{ fontSize: 'clamp(48px,6vw,80px)' }}>
            ...
            <div className="mt-7 mb-6 space-y-4 max-w-lg">
              <p className="font-body text-brand-silver/75 text-sm leading-relaxed">
                Creating cinematic experiences through visual storytelling and innovative
                production techniques. Based in Los Angeles, I've spent the last decade
                crafting narratives that bridge the gap between commercial impact and
                artistic integrity.
              </p>
              <p className="font-body text-brand-silver/65 text-sm leading-relaxed">
                My approach combines technical precision with a deep understanding of
                human emotion. From high-budget commercials to intimate independent
                documentaries, I focus on the <em>"why"</em> behind every frame.
              </p>
            </div>
            ...
            <div className="flex flex-wrap gap-4 mb-8">
              ...
              >
                <Mail size={13} className="text-brand-blue group-hover:text-brand-amber transition-colors" />
            ...
            <div className="grid grid-cols-4 gap-[1px] bg-white/[0.07] mb-8">
              {statItems.map(([val, label]) => (
                <div key={label} className="bg-brand-black py-4 flex flex-col items-center">
                  <span className="font-display text-brand-amber text-2xl leading-none">{val}</span>
                  <span className="font-body text-[9px] tracking-wider uppercase text-brand-silver/55 mt-1">{label}</span>
                </div>
              ))}
            </div>
                  ...
                  >
                    <span
                      className={`w-8 flex-shrink-0 font-body text-[10px] font-medium pt-0.5 ${
                        item.current ? 'text-brand-amber' : 'text-brand-silver/55'
                      }`}
                    >
                      '{item.year.slice(2)}
                    </span>
                    <div className="flex-1 border-t border-white/[0.08] pt-1">
                      <p className={`font-body text-xs ${item.current ? 'text-white' : 'text-brand-silver/60'}`}>
                        {item.role}
                      </p>
                      <p className="font-body text-[10px] text-brand-silver/55 mt-0.5">{item.company}</p>
                    </div>
                    {item.current && (
