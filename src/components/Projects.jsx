...
import { X, Play, Pause, Volume2, Maximize, ChevronLeft, Clock, Camera, Award } from 'lucide-react'

const projects = [
  {
    id: 1,
    title: 'The Urban Silence',
    category: 'Documentary Short',
    year: '2024',
    duration: '22 min',
    description:
      "An intimate portrait of New York's hidden communities — those who thrive in the quiet spaces between city noise. Shot over eighteen months across four boroughs, this film reveals the humanity woven into urban solitude.",
    ...
    thumb: 'https://picsum.photos/seed/urban-silence/800/450',
    full:  'https://picsum.photos/seed/urban-silence/1400/787',
  },
  {
    id: 2,
    title: 'Neon Meridian',
    ...
    duration: '90 sec',
    description:
      'A visceral brand film for a luxury automotive client, blending high-speed cinematography with contemplative long takes. Filmed across three continents in 12 days.',
    ...
    thumb: 'https://picsum.photos/seed/neon-meridian/800/450',
    full:  'https://picsum.photos/seed/neon-meridian/1400/787',
  },
  {
    id: 3,
    ...
    duration: '34 min',
    description:
      "A lyrical meditation on grief and the ocean. Two siblings return to their coastal hometown after their father's passing, unraveling decades of unspoken stories.",
    ...
    thumb: 'https://picsum.photos/seed/tides-memory/800/450',
    full:  'https://picsum.photos/seed/tides-memory/1400/787',
  },
  {
    id: 4,
    title: 'Velocity & Void',
    ...
    duration: '4:38 min',
    description:
      'A kinetic visual poem for platinum artist AXIOM. Blending practical high-speed effects with in-camera transitions, the video earned 40M+ streams in its first week.',
    ...
    thumb: 'https://picsum.photos/seed/velocity-void/800/450',
    full:  'https://picsum.photos/seed/velocity-void/1400/787',
  },
  {
    id: 5,
    ...
    thumb: 'https://picsum.photos/seed/last-canvas/800/450',
    full:  'https://picsum.photos/seed/last-canvas/1400/787',
  },
  {
    id: 6,
  ...
  return (
    ...
    >
      ...
      <div className="absolute top-3 right-3">
        <span className="font-body text-[10px] text-white/50">{project.year}</span>
      </div>
      {/* Title + duration */}
      ...
      <div className="flex-shrink-0 border-b border-white/[0.07] px-5 md:px-8 h-13 flex items-center justify-between h-[52px]">
        <div className="flex items-center gap-2.5">
          <div className="w-6 h-6 bg-brand-amber flex items-center justify-center">
            <span className="font-display text-brand-black text-sm leading-none select-none">PT</span>
          </div>
          <span className="font-display text-white tracking-[0.18em] text-xs hidden sm:block">PICTURETOWN</span>
        </div>
      ...
      <div className="flex-1 flex flex-col md:flex-row overflow-hidden">
        ...
        <div className="md:flex-1 flex flex-col bg-black min-h-[220px]">
          ...
          <div className="relative flex-1">
            ...
            <div className="absolute inset-0 flex items-center justify-center">
              ...
              >
                {playing
                  ? <Pause size={26} className="text-brand-amber" />
                  : <Play  size={26} className="text-brand-amber fill-brand-amber ml-1" />
                }
              </button>
            </div>
          ...
          <div className="bg-[#080a0c] px-4 py-3 flex items-center gap-3">
            <button onClick={() => setPlaying(v => !v)} className="text-white/60 hover:text-white transition-colors">
            ...
            <span className="font-body text-[10px] text-white/65 tabular-nums whitespace-nowrap">
              0:06:36 / {project.duration}
            </span>
            <Volume2  size={13} className="text-white/40 cursor-pointer hover:text-white/70 transition-colors" />
            <Maximize size={13} className="text-white/40 cursor-pointer hover:text-white/70 transition-colors" />
          </div>
        </div>
        ...
        <div className="md:w-[360px] lg:w-[400px] xl:w-[440px] flex-shrink-0 overflow-y-auto border-l border-white/[0.06]">
          <div className="p-6 md:p-8 space-y-6">
            ...
            {/* Title */}
            <div>
              <h2 className="font-display text-white leading-none" style={{ fontSize: 'clamp(36px,4vw,52px)' }}>
                {project.title}
              </h2>
              <p className="font-body text-brand-silver/60 text-xs mt-2">
                Directed &amp; Produced by Alex Mercer &middot; {project.year} &middot; {project.category}
              </p>
            </div>
              ...
              <div className="space-y-2.5">
                {Object.entries(project.specs).map(([key, val]) => (
                  <div key={key} className="flex items-center justify-between gap-4">
                    <span className="font-body text-brand-silver/60 text-xs">{key}</span>
                    <span className="font-body text-brand-silver/80 text-xs text-right">{val}</span>
                  </div>
                ))}
              </div>
            </div>
                ...
                <ul className="space-y-2">
                  {project.awards.map(award => (
                    <li key={award} className="flex items-start gap-2.5">
                      ...
                      <span className="font-body text-brand-silver/65 text-xs leading-snug">{award}</span>
                    </li>
                  ))}
                </ul>
              </div>
      ...
      <div className="max-w-7xl mx-auto px-6 md:px-10">
        ...
        <div className="flex items-end justify-between mb-10 md:mb-14">
          ...
          <p className="hidden md:block font-body text-brand-silver/60 text-sm max-w-[240px] text-right leading-relaxed">
            Narrative films, brand commercials, and documentary work spanning 12 years.
          </p>
        </div>
